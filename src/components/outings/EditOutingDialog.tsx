...
import { useUpdateOuting, Outing, OutingType } from "@/hooks/useOutings";
...
const EditOutingDialog = ({ outing }: EditOutingDialogProps) => {
  ...
  const form = useForm<EditOutingFormData>({
    resolver: zodResolver(editOutingSchema),
    defaultValues: {
      title: outing.title,
  ...
  const onSubmit = (data: EditOutingFormData) => {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <Button
              ...
              variant="ocean"
              className="w-full"
              disabled={updateOuting.isPending}
            >
              {updateOuting.isPending ? "Enregistrement..." : "Enregistrer les modifications"}
            </Button>
          </form>
        </Form>
      </DialogContent>
