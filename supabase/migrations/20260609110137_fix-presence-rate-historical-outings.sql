-- Fix presenceRate: historical outings count as 100% present
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'presenceRate', (
      -- Historical outings: all participants are present (100%)
      -- Regular outings: ratio of is_present=true over confirmed
      WITH outing_presence AS (
        SELECT
          COUNT(hop.member_id)::numeric AS presents,
          COUNT(hop.member_id)::numeric AS inscrits
        FROM outings o
        JOIN historical_outing_participants hop ON hop.outing_id = o.id
        WHERE o.date_time >= start_of_year AND o.date_time <= end_of_year
          AND o.date_time < NOW()
        UNION ALL
        SELECT
          COUNT(CASE WHEN r.is_present = true THEN 1 END)::numeric AS presents,
          COUNT(*)::numeric AS inscrits
        FROM outings o
        JOIN reservations r ON r.outing_id = o.id
        WHERE o.date_time >= start_of_year AND o.date_time <= end_of_year
          AND o.date_time < NOW()
      ...
      SELECT COALESCE(ROUND(SUM(presents) / NULLIF(SUM(inscrits), 0) * 100), 0)
      FROM outing_presence
    ),
    'totalParticipants', (
